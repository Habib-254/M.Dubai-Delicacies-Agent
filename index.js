...
async function startBot() {
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'open') console.log('✅ M.DUBAI AI IS ONLINE!');
        if (connection === 'close') {
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        if (orderStates[sender]?.step === 'WAITING_FOR_ADDRESS') {
            ...
            // Match the exact format of your M.Dubai Admin Panel
            const m.DubaiOrder = {
            ...
            try {
                await fetch(`${FIREBASE_URL}/orders.json`, {
                    ...
                    body: JSON.stringify(m.DubaiOrder)
                });
            } catch (error) {
        ...
        // --- DYNAMIC MENU FEATURE ---
            ...
            let menuMessage = "🍔 *M.DUBAI LIVE MENU* 🍕\n\n";
            currentMenu.forEach(item => {
                menuMessage += `🔸 *${item.name}* - ksh${item.price}\n`;
            });
